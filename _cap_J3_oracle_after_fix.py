# -*- coding: utf-8 -*-
# Oracle vitest byte-safe -> arquivo; print so veredito ascii.
import os, re, subprocess, time
BASE = "C:/wamp64/www/news-ready-portal/opencode/news-ready-portal"
OUT = os.path.join(BASE, "_oracle_alerts_J3.out")
...
try:
    with open(OUT, "wb") as f:
        r = subprocess.run(["cmd", "/c", vit, "run", "src/test/alerts.test.tsx",
                           ...
                           env=env, timeout=200_000)
    print("[oracle] exit:", r.returncode, "| %.1fs" % (time.time() - t0))
...
for l in rawO.splitlines():
    ...
    if re.search(r"(Test Files|Tests |PASS|FAIL|passed|failed|ReferenceError"
                 r"|nullapsed|Element type|Cannot read|Unable to find|✓|×"
                 r"|toHaveTextContent|toEqual|Received)", a):
        print(a[:160])
